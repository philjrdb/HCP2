...
RT_check_grid = [3 3 6 6 6 6; 2 2 8 8 8 8; 2 2 8 8 8 8; 0 0 2 2 2 2; 0 0 2 2 2 2]; % # post-block val/inf questions
...
work_folder = ['G:\Current\Student folders\Liz\HCP2 analyses\Fixed Utility'];
...
copy_file = {'Phil_HCPextract_v6.m' 'AvBbinning.m' 'bin_rate2.m' 'HCP_sumfig2.m' ...
...
for r = 1:length(HCP_aggr)
  ...
  HCP_aggr(r).sonaID = HCP_aggr(r).raw{1,3}.sona_id;
  HCP_aggr(r).Group = HCP_aggr(r).raw{1,3}.group;
...
%      min(HCP_aggr(r).raw{phase_idx(p),3}.all_clicks.timestamp)])/1000;
    ...
    if ismember(p,pun_trials) % if punished phase
      ...
      HCP_aggr(r).Block.ExpPun_ShipRate(p) = size(HCP_aggr(r).Block.PunShips{p},1)/...
        (HCP_aggr(r).Block.BinPunRate{p}(1)*HCP_aggr(r).Block.BinTime{p}(1));
      HCP_aggr(r).Block.ExpUnp_ShipRate(p) = size(HCP_aggr(r).Block.UnpShips{p},1)/...
        (HCP_aggr(r).Block.BinUnpRate{p}(1)*HCP_aggr(r).Block.BinTime{p}(1));
%       fprintf([' - experienced Pun/UnpShip probability: ' num2str(round(HCP_aggr(r).Block.ExpPun_ShipRate(p),3)) ...
%         '/' num2str(round(HCP_aggr(r).Block.ExpUnp_ShipRate(p),3))]);
  ...
  fprintf(['Experienced ITI Pun/Unp -> Ship probability: ' ...
    num2str(round(sum(cellfun(@(x) size(x,1),HCP_aggr(r).Block.PunShips(pun_trials)))/sum(cellfun(@(x) x(1),...
    HCP_aggr(r).Block.BinPunRate(pun_trials)).*cellfun(@(x) x(1),HCP_aggr(r).Block.BinTime(pun_trials))),3)) '/'...
    num2str(round(sum(cellfun(@(x) size(x,1),HCP_aggr(r).Block.UnpShips(pun_trials)))/sum(cellfun(@(x) x(1),...
    HCP_aggr(r).Block.BinUnpRate(pun_trials)).*cellfun(@(x) x(1),HCP_aggr(r).Block.BinTime(pun_trials))),3)) '\n']);
  
  fprintf(['Experienced Pun/Unp (all "trades") -> Ship probability: ' ...
    num2str(round(sum(cellfun(@(x) size(x,1),HCP_aggr(r).Block.PunShips(pun_trials)))/sum(cellfun(@(x) size(x,1),...
    HCP_aggr(r).Block.PunRew(pun_trials))),3)) '/'...
    num2str(round(sum(cellfun(@(x) size(x,1),HCP_aggr(r).Block.UnpShips(pun_trials)))/sum(cellfun(@(x) size(x,1),...
    HCP_aggr(r).Block.UnpRew(pun_trials))),3)) '\n']);
...
%% Finish up
N = length(HCP_aggr);
excl = sum(vertcat(HCP_aggr(:).Exclude));
fprintf(['\nHCP extracted. N = ' num2str(N) ' (-' ...
  num2str(excl) ' with failed checks). Valid N = ' num2str(N-excl) '. ']);

% Check for missing sonaID/duplicates
idx = cellfun('isempty', {HCP_aggr(:).sonaID});
if any(idx)
  m_idx = find(idx);
  fprintf(['MISSING SONA ID from rows: ' ...
    strjoin(arrayfun(@(x) int2str(x),m_idx,'UniformOutput',false),',') '\n']);
  for i = 1:length(m_idx)
    HCP_aggr(m_idx(i)).sonaID = 'MISSING';
  end
end
v_idx = find(~idx);
sonaIDs = str2num(vertcat(HCP_aggr(v_idx).sonaID));
if length(unique(sonaIDs)) < length(v_idx)
  sonaID_2 = [];
  i_win = 1:length(v_idx);

  for a = i_win
    if ismember(sonaIDs(a),sonaIDs(setdiff(i_win,a)))
      if ~ismember(sonaIDs(a),sonaID_2)
        sonaID_2(end+1) = sonaIDs(a);
      end
    end
  end
  n_dupl = length(sonaID_2);
  fprintf([int2str(n_dupl) ' DUPLICATES!\n']);
  
  for a = 1:n_dupl
    h_rows = v_idx(ismember(sonaIDs,sonaID_2(a)));
    fprintf([num2str(sonaID_2(a)) ': rows ' ...
      strjoin(arrayfun(@(x) int2str(x),h_rows,'UniformOutput',false),',') ...
      '(' strjoin({HCP_aggr(h_rows).json},',') ')\n']);
  end
else
  fprintf('No duplicates. ');
end
